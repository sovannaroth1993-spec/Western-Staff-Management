...
export default function ElectricityTracker({
  ...
  // Trigger Edit record
  const handleEditInit = (record: ElectricityRecord) => {
  ...
  const handleSaveRecord = (e: React.FormEvent) => {
    ...
    const existingMonth = electricityRecords.find(
    ...
    if (editingId) {
      // Edit existing
      const updated = electricityRecords.map(r => {
        if (r.id === editingId) {
          return {
            ...r,
      ...
      setElectricityRecords([...electricityRecords, newRecord]);
    }
  ...
  const handleDeleteRecord = (id: string) => {
    setElectricityRecords(electricityRecords.filter(r => r.id !== id));
    setDeleteId(null);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Upper bar & Title */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-amber-50/40 border border-amber-100 p-5 rounded-2xl">
        <div className="flex items-start gap-3">
          <span className="p-3 bg-amber-500/10 text-amber-600 rounded-xl mt-1 shrink-0">
            <Zap className="w-6 h-6 fill-amber-500/30 text-amber-500" />
          </span>
          <div>
            <h3 className="text-lg font-black text-slate-800">
              ការគ្រប់គ្រងការប្រើប្រាស់អគ្គិសនីប្រចាំខែ (Monthly Electricity Comparison in USD)
            </h3>
            <p className="text-xs font-semibold text-slate-400 mt-0.5">
              ប្រៀបធៀប និងតាមដានរបាយការណ៍ចំណាយភ្លើង ចន្លោះការប្រើប្រាស់ខែមុន និងខែបន្ទាប់គិតជាដុល្លារ ($)
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 bg-amber-500 hover:bg-amber-600 text-white font-black text-xs px-4 py-2.5 rounded-xl transition shadow-md shadow-amber-100 shrink-0 cursor-pointer self-start sm:self-auto"
        >
          <Plus className="w-4 h-4" />
          បញ្ចូលទិន្នន័យអគ្គិសនី ($)
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-slate-900 text-white rounded-2xl p-5 border border-slate-800 relative overflow-hidden shadow-lg flex flex-col justify-between min-h-[160px]">
            <div className="flex items-center justify-between">
              <div>
                <span className="bg-amber-500 text-slate-950 text-[10px] font-black tracking-wider px-2 py-0.5 rounded-full uppercase">
                  ការប្រើប្រាស់ចុងក្រោយបង្អស់ (Latest)
                </span>
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200/60 shadow-md flex flex-col justify-between min-h-[160px]">
            <div className="flex items-center justify-between">
              <div>
                <span className="bg-emerald-50 text-emerald-700 border border-emerald-100 text-[10px] font-black tracking-wider px-2 py-0.5 rounded-full uppercase">
                  ចំណាយលើអគ្គិសនីសរុប (All Months)
                </span>
            ...
            <div className="w-full h-1 bg-emerald-500 absolute bottom-0 left-0" />
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200/60 shadow-md flex flex-col justify-between min-h-[160px]">
            <div className="flex items-center justify-between">
              <div>
                <span className="bg-amber-50 text-amber-700 border border-amber-100 text-[10px] font-black tracking-wider px-2 py-0.5 rounded-full uppercase">
            ...
            <div className="mt-2 space-y-1.5">
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-extrabold text-emerald-650 text-emerald-600">
              ...
              <div className="flex items-center justify-between text-xs border-t border-slate-100 pt-1.5 mt-1">
                <span className="text-slate-400 font-bold">មធ្យមភាគចំណាយ៖</span>
      ...
      {sortedRecords.length === 0 ? (
        <div className="text-center py-24 bg-slate-50/50 border border-dashed border-slate-200 rounded-2xl p-6">
          <Zap className="w-14 h-14 text-slate-300 mx-auto mb-2.5" />
          <h4 className="text-sm font-extrabold text-slate-700">មិនទាន់មានទិន្នន័យចំណាយអគ្គិសនីប្រចាំខែទេ</h4>
          <p className="text-xs text-slate-400 font-semibold mt-1 max-w-sm mx-auto">
            ចុចលើប៊ូតុង <span className="underline text-amber-500 font-bold">"បន្ថែមទិន្នន័យអគ្គិសនី ($)"</span> ខាងលើ ដើម្បីបញ្ចូលទិន្នន័យប្រៀបធៀបចំណាយអគ្គិសនីប្រចាំខែ!
          </p>
        </div>
      ) : (
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="p-4 bg-slate-50 border-b border-gray-200/60 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <h4 className="text-sm font-black text-slate-700 flex items-center gap-2">
              ...
              ប្រវត្តិនៃការចុះបញ្ជី និងប្រៀបធៀបចំណាយអគ្គិសនី (Electricity Expense Comparison)
            </h4>
            <span className="text-[10px] bg-slate-200 text-slate-650 font-black px-2.5 py-1 rounded-lg uppercase tracking-wider text-slate-700">
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-slate-50/50 transition duration-150">
                      ...
                      <td className="py-4 px-4 text-center">
                        <span className="inline-block bg-amber-50 text-amber-700 font-extrabold px-2.5 py-1 rounded-lg border border-amber-100 whitespace-nowrap">
                      ...
                      <td className="py-4 px-5 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            ...
                            className="p-1.5 text-slate-400 hover:text-amber-500 bg-white hover:bg-amber-50 border border-slate-200 rounded-lg transition"
                            title="កែប្រែ"
                          >
                          ...
                          <button
                            ...
                            className="p-1.5 text-slate-400 hover:text-rose-600 bg-white hover:bg-rose-50 border border-slate-200 rounded-lg transition"
                            title="លុបចោល"
                          >
        ...
        {isFormOpen && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 font-sans">
            <motion.div
              ...
              id="electric-form-modal"
            >
              {/* Header */}
              <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between">
                <h4 className="text-sm font-black flex items-center gap-2">
                  <Zap className="w-4 h-4 text-amber-400 fill-amber-400/20" />
                  {editingId ? 'កែប្រែរបាយការណ៍អគ្គិសនីប្រចាំខែ' : 'បញ្ចូលទិន្នន័យអគ្គិសនីប្រចាំខែថ្មី'}
                </h4>
                <button 
              ...
              <form onSubmit={handleSaveRecord}>
                <div className="p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2">
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-50 border border-slate-200 font-bold p-3 rounded-xl focus:ring-1 focus:ring-amber-400 focus:outline-none text-slate-800 text-sm"
                      />
                    </div>

                    {/* Previous month consumption cost input */}
                    <div>
                      <label className="block text-slate-500 font-extrabold text-xs mb-1.5 text-slate-650">
                      ...
                      <div className="relative">
                        ...
                        <input 
                          ...
                          className="w-full bg-slate-50 border border-slate-200 font-black pl-8 p-3 rounded-xl focus:ring-1 focus:ring-amber-400 focus:outline-none text-slate-800 text-sm text-center"
                        />
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    <textarea 
                      placeholder="ឧទាហរណ៍៖ ប្រជុំច្រើន, មានកម្មវិធីធំ..."
                      value={formNotes}
                      onChange={(e) => setFormNotes(e.target.value)}
                      rows={2}
                      className="w-full bg-slate-50 border border-slate-200 text-slate-700 font-semibold p-3 rounded-xl focus:ring-1 focus:ring-amber-400 focus:outline-none text-xs"
                    />
                  </div>
                </div>
                ...
                <div className="bg-slate-50 px-6 py-4 flex justify-end gap-2 border-t border-slate-100">
                  ...
                  <button
                    type="submit"
                    className="bg-amber-500 hover:bg-amber-600 text-white text-xs font-extrabold px-5 py-2.5 rounded-xl transition shadow-md shadow-amber-100 cursor-pointer"
                  >
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 font-sans">
            ...
            >
              ...
              <div className="p-6 text-center">
                ...
                <h5 className="text-sm font-bold text-slate-800 mb-1">តើអ្នកពិតជាចង់លុបទិន្នន័យអគ្គិសនីនេះទេ?</h5>
                <p className="text-xs text-slate-400 font-semibold">
                  ទិន្នន័យប្រើប្រាស់សម្រាប់ខែ '{formatKhmerMonth(electricityRecords.find(r => r.id === deleteId)?.monthYear || '')}' នឹងត្រូវលុបជាអចិន្ត្រៃយ៍។
                </p>
              </div>
